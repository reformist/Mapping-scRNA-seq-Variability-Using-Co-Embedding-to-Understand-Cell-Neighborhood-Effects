...
protein_adata = anndata.AnnData(protein[protein_features].to_numpy(), dtype=np.float32)
...
rna = mmread(
    "CODEX_RNA_seq/data/raw_data/tonsil/tonsil_rna_counts.txt"
)  # rna count as sparse matrix, 10k cells (RNA)
rna_names = pd.read_csv("CODEX_RNA_seq/data/raw_data/tonsil/tonsil_rna_names.csv")[
    "names"
].to_numpy()
# convert to AnnData
rna_adata = anndata.AnnData(rna.tocsr(), dtype=np.float32)
...
adata_1 = adata_1[
    adata_1.obs["cell_types"].argsort(), :
...
adata_2 = adata_2[
    adata_2.obs["cell_types"].argsort(), :
]  # sort by cell types for easier visualization
...
if "gene" in adata_1.var.columns and np.array_equal(
    adata_1.var["gene"].values, (adata_1.var.index.values)
):
    adata_1.var.drop(columns="gene", inplace=True)
if "gene" in adata_2.var.columns and np.array_equal(
    adata_2.var["gene"].values, (adata_2.var.index.values)
):
...
# %%
adata_1 = adata_1[adata_1.obs.sort_values(by=["cell_types"]).index]

adata_2 = adata_2[adata_2.obs.sort_values(by=["cell_types"]).index]
...
if plot_flag:
    sc.pl.scatter(
        adata_2_prot[adata_2_prot.obs["major_cell_types"] == "T-reg"],
        x="X",
        y="Y",
        color="cell_types",
