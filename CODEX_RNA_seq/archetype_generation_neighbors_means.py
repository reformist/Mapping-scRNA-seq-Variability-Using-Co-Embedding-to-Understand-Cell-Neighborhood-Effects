...
num_of_protein_cells = adata_2_prot.shape[0]
if num_of_protein_cells < 30000:
    raise ValueError(
        f"Number of protein cells is less than 30000, are you sure this is the mouse CODEX data?"
    )
num_rna_cells = 6000
...
# %%
neighbor_sums = connectivities.todense().dot(
    adata_2_prot.X.todense() if issparse(adata_2_prot.X) else adata_2_prot.X
)  # sum the neighbors
...
normalized_data = zscore(
    neighbor_means.toarrray() if issparse(neighbor_means) else neighbor_means, axis=0
)
...
palette = sns.color_palette(
    "tab10", num_clusters
)  # "tab10" is a good color map, you can choose others too
...
if plot_flag:
    ...
    adata_prot_cn_concat = anndata.concat(
...
if adata_2_prot.X.shape[1] == neighbor_means.shape[1]:
    new_X = np.hstack(
        [adata_2_prot.X.todense() if issparse(adata_2_prot.X) else adata_2_prot.X, neighbor_means]
    )
...
for i in range(num_archetypes):
    ...
    rna_archetypes.append(
        archetype_cells.X.toarray() if issparse(archetype_cells.X) else archetype_cells.X
    )
        ...
        for j, archetype2 in enumerate(archetypes_space2):
            ...
            try:
                ...
                gw_dist = ot.gromov.entropic_gromov_wasserstein2(
                    C1, C2, p, q, loss_type, epsilon=epsilon, max_iter=max_iter, verbose=False
                )
...
print(
    f"Percentage of agreement between OT and Best Matching: {np.mean(matched_indices_protein_ot == best_protein_archetype_order) * 100:.2f}%"
)
