...
log_transformed_distances = (
    distances + 1
)  # since the distances are not normally distributed, log-transform them
...
(
    adata_2_prot.obsm["COVET"],
    adata_2_prot.obsm["COVET_SQRT"],
    adata_2_prot.uns["CovGenes"],
) = compute_covet(exp_adata_2_prot, k=21, spatial_key="spatial")
adata_2_prot.obsm["COVET_SQRT"] = adata_2_prot.obsm["COVET_SQRT"].reshape(
    adata_2_prot.obsm["COVET_SQRT"].shape[0], -1
)
...
sc.pl.scatter(
    ...
    y="Y",
    color=["CN", "cell_type"],
    title=["spatial and CN as color", "spatial and cell type as color"],
)
# del conv_sqrt
...
assert (
    sil_score < 0.5
), "Silhouette score is too high the CN labels should not depend on the spatial location"
...
sc.pl.pca(
    conv_sqrt, color=["cell_types"], title=["PCA - Cell Types", "PCA - Original Protein Clusters"]
)

# Plot the UMAP embedding
sc.pl.umap(
    conv_sqrt, color=["cell_types"], title=["UMAP - Cell Types", "UMAP - Original Protein Clusters"]
)
...
if plot_flag:
    ...
    sns.heatmap(
        adata_2_prot.obsm["COVET_SQRT"]
    )  # less noisy because we mean, makes sense as we made sure each cell is of same cell type
...
class ClusterPreservingTrainingPlan(TrainingPlan):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if not hasattr(self, "total_steps"):
            ...
            self.plot_interval = max(
                1, (steps_per_epoch * self.trainer.max_epochs) // self.plot_x_times
            )
        ...
        x = batch["X"]
        inference_outputs = self.module.inference(x, batch_index=batch["batch"], n_samples=1)
        ...
        if torch.mean(dynamic_range) < 0.5:
            dynamic_range_loss = -10000 * torch.log(
                torch.mean(dynamic_range)
            )  # Encourage larger dynamic range
        else:
    ...
    def calculate_batch_silhouette(self, z, batch_labels):
        ...
        try:
            ...
            latent = (latent - latent.min(axis=0)) / (
                latent.max(axis=0) - latent.min(axis=0) + 1e-8
            )
...
def improved_triplet_cluster_loss(z, labels, margin=1.0, var_weight=0.1, use_semi_hard=True):
    ...
    mean_pos_dist = torch.sum(pairwise_dist * positive_mask.float(), dim=1) / (
...
def triplet_cluster_loss(z, labels, margin=1.0):
    ...
    hardest_positives = torch.nanquantile(
        masked_dist_pos, 0.9, dim=1
    )  # 90th percentile distance to positive
    hardest_negatives = torch.nanquantile(
        masked_dist_neg, 0.1, dim=1
    )  # 10th percentile distance to negative
...
combined_data = np.concatenate(
    [adata_2_prot.X.todense() if issparse(adata_2_prot.X) else adata_2_prot.X, COVET_SQRT], axis=1
)
...
target_silhouette = silhouette_avg = silhouette_score(
    adata_2_prot.obsm["X_pca"], adata_2_prot.obs["orig_prot_clusters"].astype(int)
)
...
sc.pl.umap(
    combined_adata,
    color=["cell_types", "orig_prot_clusters"],
    title=["UMAP - Cell Types", "UMAP - Original Protein Clusters"],
)
...
sc.pl.umap(
    combined_adata,
    color=["cell_types", "orig_prot_clusters"],
    title=["latent - Cell Types", "latent - original prot Clustering"],
)
...
sc.pl.umap(
    ...
    color=["cell_types", "orig_prot_clusters"],
    title=[
        "UMAP of Original Protein Data - Cell Types",
...
sc.pl.umap(
    adata_largest_cluster,
    color="cell_types",
    title=f"Largest Cluster ({largest_cluster}) - Colored by Cell Types",
)
...
silhouette_avg = silhouette_score(
    combined_adata.obsm["X_scVI"], combined_adata.obs["orig_prot_clusters"].astype(int)
)
...
for archetypes_prot, archetypes_rna in tqdm(
    ...
    archetypes_dim_prot = archetypes_prot.shape[
        1
    ]  # these dimensions are not 25 anymore, made them 50
...
archetype_distances = cdist(cells_archetype_vec_rna, cells_archetype_vec_prot, metric="correlation")
if plot_flag:
    plt.figure(figsize=(10, 5))
    plt.subplot(1, 2, 1)
    ...
    min_values_highlight[
        archetype_distances.argmin(axis=0), range(len(archetype_distances.argmin(axis=0)))
    ] = 100
...
if plot_flag:
    ...
    plt.title("Protein Archetypes")
    plt.suptitle(
        "The more similar the better, means that the archtypes are aligned in explaining different cell types"
    )
    plt.show()
...
# %%
len(data_points_rna_plot), len(ordered_best_rna_archetype), len(samples_cell_types_rna_plot), len(
...
if plot_flag:
    ...
    if show_ground_truth:
        ...
        samples_cell_types_rna = [
            current_cell_types_list[i] for i in data_point_archetype_indices_rna
        ]
        samples_cell_types_prot = [
            current_cell_types_list[i] for i in data_point_archetype_indices_prot
        ]
