...
def load_protein_data(data_dir):
    ...
    protein = pd.read_csv(data_dir / "citeseq_pbmc/pro.csv") # 10k cells (protein)
    # convert to AnnData
    ...
    metadata = pd.read_csv(data_dir / "citeseq_pbmc/meta.csv")
    ...
    protein_adata.obs['cell_types'] = labels_l1
...
def load_rna_data(data_dir):
    ...
    # read in RNA data
    rna = mmread(data_dir / "citeseq_pbmc/rna.txt") # rna count as sparse matrix, 10k cells (RNA)
    rna_names = pd.read_csv(data_dir / 'citeseq_pbmc/citeseq_rna_names.csv')['names'].to_numpy()
    # convert to AnnData
    metadata = pd.read_csv(data_dir / "citeseq_pbmc/meta.csv")
    labels_l1 = metadata['celltype.l1'].to_numpy()
    labels_l2 = metadata['celltype.l2'].to_numpy()
    ...
    rna_adata.obs['cell_types'] = labels_l1
...
sc.pp.subsample(protein_adata, n_obs=10000)
